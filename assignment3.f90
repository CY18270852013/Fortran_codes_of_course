program InterpolateData
    ...
    ! 参数定义
    integer, parameter :: nx_source = 128  ! 源数据经度格点数
    integer, parameter :: ny_source = 108   ! 源数据纬度格点数
    ...
    ! 将插值结果写入文件
    open(unit=3, file='output.txt', status='replace')
    do j = 1, ny_target
        do i = 1, nx_target
            write(3, *) lon_target(i), lat_target(j), targetData(i, j)
        end do
    end do
    close(3)
